...
class BP_Flickr extends WP_Widget
{
	...
	function BP_Flickr(){
		...
		$this->WP_Widget('flickr_widget', __('Feather Flickr','dsf'), $widget_ops);
    }
    ...
    function widget($args , $instance)
    {
		...
		<div class="flickr-wrapper">
        	<div class="flexslider flickr-widget-<?php echo $flickr_rand; ?>">
				
				<ul class="slides"></ul>


        	</div><!-- end flickr widget -->
    	</div><!-- end flickr container -->
    ...
    function print_inline_content()
    {
    	if(wp_script_is($this->handle , 'done'))
    	{
    		...
    		<script type="text/javascript">
    			jQuery('.flickr-widget-<?php echo $this->flickr_rand; ?> ul.slides').html('').jflickrfeed({
					        limit: <?php echo $this->limit; ?>,
					        qstrings: {
					          id: '<?php echo $this->id; ?>'
					        },
					        itemTemplate: 
					        '<li><a href="{{link}}"><img src="{{image_m}}" alt="{{title}}" /></a></li>'
					      	});    		
    		</script>
